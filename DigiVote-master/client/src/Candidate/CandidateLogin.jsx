...
class CandidateLogin extends Component {
  ...
  SignUp = async (event) => {
    ...
    if (
      ...
      await contract.methods
        .findCandidate(this.state.username)
        .call()
      ...
      if (!isUsernamePresent) {
        ...
        await contract.methods
          .setCandidateCredentials(
            ...
            this.state.ipfsCredentialsHash
          )
        ...
        //function for sending hash to the blockchain
        await contract.methods
          .setCandidatePersonal(
            PersonalData.Username,
            this.state.ipfsPersonalHash
          )
          .send({ from: accounts[2], gas: 6721975 })
          .then((receipt) => {
            console.log(receipt);
        ...
        let result;
        await contract.methods
          .getCandidatePersonal(PersonalData.Username)
          .call()
          .then((res) => (result = res))
          .catch(console.error);
  ...
  SignIn = async (event) => {
    ...
    await contract.methods
      .findCandidate(this.state.inputUsername)
      .call()
      .then((res) => (result = res))
      .catch(console.error);
    console.log(result);
    ...
    if (result) {
      let hash;
      await contract.methods
        .getCandidateCredentials(this.state.inputUsername)
        .call()
        .then((res) => (hash = res))
        .catch(console.error);
      ...
      if (ipfsCredentailsData.Password === this.state.inputPassword) {
        await web3.eth.personal
          .unlockAccount(
            ipfsCredentailsData.Address,
            ipfsCredentailsData.Password,
            600
          )
        ...
        await contract.methods
          .getCandidatePersonal(this.state.inputUsername)
          .call()
  ...
  updateConsitituency = (evt) =>
    this.setState({ constituency: evt.target.value });

  updateDOB = (val) => this.setState({ DOB: val });

  updateElectionParty = (evt) =>
    this.setState({ electionParty: evt.target.value });

  updatePartySymbol = (evt) =>
    this.setState({ partySymbol: evt.target.files[0] });

  render() {
    console.log("CandidateLogin");
    return (
      <div className="testClass">
        <Fade in={this.state.loaderStart === true} unmountOnExit>
          <Grid
            container
            direction="row"
            justify="center"
        ...
        >
          {this.state.goToSignUp ? (
            <Paper elevation={2} className="RegisterCard2">
            ...
            <Paper elevation={2} className="loginCard">
              <Typography variant="h4" gutterBottom>
                Candidate Login
              </Typography>
              ...
              <Home
                className="HomeIcon"
                onClick={() => this.props.updateHomeState(0, null)}
              />
            </Paper>
          )}
        </Grid>
